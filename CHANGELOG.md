...
- `esc` out of a non-matching SEARCH query no longer leaves NORMAL holding a permanently empty
  list, where every navigation key did nothing and a verb acted on no selection at all. Type a
  query that matches nothing, press `esc`, and the list is now back. `disable-search` stops future
  matching but never re-filters, so `clear-query` alone changed nothing and the match set stayed
  frozen at whatever SEARCH had narrowed it to. `esc` now runs `clear-query+search()` to force the
  re-match, and those two actions sit on the key binding rather than inside the `transform:` that
  decides what `esc` means, because fzf ignores a `search()` that a `transform:` emits.
